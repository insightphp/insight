  ...
  <Listbox v-model="selectedColor" v-slot="{ open }">
    <div class="relative w-full select" :class="{ 'open': open }">
      <ListboxButton class="w-full relative text-left select-button" :class="{ 'open': open, 'has-error': hasError }">
        <div class="px-3 py-2">
          <div v-if="selectedColor" class="h-5 w-full rounded-md" :style="{ backgroundColor: selectedColor }"></div>
      ...
      <Transition enter-from-class="opacity-0" enter-to-class="opacity-100" enter-active-class="transition-opacity duration-150" leave-active-class="transition-opacity duration-150" leave-from-class="opacity-100" leave-to-class="opacity-0">
        <ListboxOptions class="w-full focus:outline-none shadow-lg bg-white z-10 absolute max-h-60 overflow-x-hidden overflow-y-auto w-full border rounded-b-lg -mt-1.5 select-option-list" :class="[ hasError ? 'border-danger-300' : 'border-gray-300' ]">
          <ListboxOption v-for="option in options" :key="option" :value="option" v-slot="{ active, selected }">
            <li
                class="relative px-4 py-2 cursor-pointer flex items-center justify-center"
